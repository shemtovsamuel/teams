...
{
    char *message = calloc(strlen("/list_channel") + 1 + strlen(messages) + 4,
        sizeof(char));

    if (message == NULL)
        return (NULL);
    sprintf(message, "/list_channel%s;\r\n", messages);
    return (message);
}
...
static char *find_channels(client_t *client, char ***csv)
{
    ...
    for (int i = 0; csv[i + 1]; i++) {
        if (strcmp(csv[i + 1][3], client->team_uuid) != 0)
            continue;
...
void list_channel(server_t *server USELESS, client_t *client)
{
    char ***csv = parse_csv_file(CHANNELS_SAVE_FILE);

    if (csv == NULL)
        return;
    add_to_client_queue(client, find_channels(client, csv), false);
    free_csv_array(csv);
}